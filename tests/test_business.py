...
def test_ensure_running_already_running_suppresses_indexing(monkeypatch, tmp_path):
    calls = []
    def handler(argv, kw):
        calls.append(argv)
    ...
    _mock_subprocess(monkeypatch, handler)
    _make_vm_in(tmp_path).ensure_running()

    assert (tmp_path / ".metadata_never_index").exists()
    assert any("tmutil" in str(c) and "addexclusion" in str(c) for c in calls)


def test_ensure_running_stopped_suppresses_indexing(monkeypatch, tmp_path):
    calls = []
    def handler(argv, kw):
        calls.append(argv)
        if argv[:3] == ["limactl", "list", "--format"]:
...
def test_ensure_running_new_vm_suppresses_indexing(monkeypatch, tmp_path):
    calls = []
    def handler(argv, kw):
        calls.append(argv)
        if argv[:3] == ["limactl", "list", "--format"]:
...
def test_destroy_restores_host_indexing(monkeypatch, tmp_path):
    marker = tmp_path / ".metadata_never_index"
    marker.touch()

    calls = []
    _mock_subprocess(monkeypatch, lambda argv, kw: (calls.append(argv) or _R(0)))
    _make_vm_in(tmp_path).destroy()

    assert not marker.exists()
    assert any("tmutil" in str(c) and "removeexclusion" in str(c) for c in calls)
